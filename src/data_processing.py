...
def load_cbo_data(filepath: str | Path) -> pd.DataFrame:
...
def build_correspondence_table(cbo_hierarquia_df: pd.DataFrame) -> pd.DataFrame:
    correspondencia = (
        cbo_hierarquia_df
        .groupby('COD_PNAD')
        .agg(
            ...
            TITULO_REFERENCIA=('TITULO', 'first'),
